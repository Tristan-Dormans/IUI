...
function suggestRecipe(busyHours) {
    ...
    const suitableRecipes = recipes.filter(recipe => recipe.cooking_time <= freeHours * 60);

    suitableRecipes.sort((a, b) => a.cooking_time - b.cooking_time);
...
document.getElementById('generate-recipe').addEventListener('click', (e) => {
    ...
    if (suggestedRecipe) {
        ...
        alert(`Suggested Recipe: ${suggestedRecipe.name} (Cooking time: ${suggestedRecipe.cooking_time} minutes)`);
    } else {
